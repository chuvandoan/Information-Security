...
print('1.2:', df['label'].value_counts()['Junichiro Koizumi'] / df['label'].value_counts().sum())

mean_persons = df.groupby('label').mean()
print('1.3:', mean_persons.loc['Hugo Chavez'][0])
...
ab = mean_persons.loc[['Jacques Chirac', 'Serena Williams']].prod(axis=0).sum()
a = sqrt(mean_persons.loc[['Jacques Chirac', 'Jacques Chirac']].prod(axis=0).sum())
b = sqrt(mean_persons.loc[['Serena Williams', 'Serena Williams']].prod(axis=0).sum())
print('1.5:', ab / (a * b))
...
# Binary search to find the number of components
l = 0
